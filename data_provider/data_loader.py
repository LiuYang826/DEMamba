...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        hour_mapping = {9: 0, 10: 1, 11: 2, 13: 3, 14: 4, 15: 5}
        min_mapping = {0: 0, 5: 1, 10: 2, 15: 3, 20: 4, 25: 5, 30: 6, 35: 7, 40: 8, 45: 9, 50: 10, 55: 11}
        df_stamp = df_raw[['date']][border1:border2]
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        if self.timeenc == 0:
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)
            ...
            df_stamp['weekday'] = df_stamp.date.apply(lambda row: row.weekday(), 1)
            df_stamp['hour'] = df_stamp.date.apply(lambda row: row.hour, 1).map(hour_mapping)
            df_stamp['min'] = df_stamp.date.apply(lambda row: row.minute, 1).map(min_mapping)
            data_stamp = df_stamp.drop(['date'], 1).values
        elif self.timeenc == 3:
            df_stamp['month'] = df_stamp.date.apply(lambda row: row.month, 1)-1
            df_stamp['day'] = df_stamp.date.apply(lambda row: row.day, 1)-1
